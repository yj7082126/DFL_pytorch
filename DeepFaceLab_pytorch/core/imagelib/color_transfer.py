...
def LaplacianPyramid(img, leveln):
    gpA = GaussianPyramid(img, leveln)
...
def blend_pyramid(LPA, LPB, MP, blend_levels = 2):
    blended = []
...
def get_levels(resolution):
    if resolution % 2 == 1:
        raise Exception("Disney Fail: resolution not dividable by 2")
    else:
        level = 0
        while resolution % 2 == 0:
            ...
            level += 1
        blend_level = level - 2
        return level, blend_level

def disney_color_transfer(target, source, mask, level_auto=True, 
                          level=5, blend_level=3):
    if level_auto:
        level, blend_level = get_levels(mask.shape[0])

    mask_a = mask.copy()
    mask_a[mask_a>0] = 1.0
    mp = GaussianPyramid(mask_a, level)

    masked_target = target.copy() * mask_a
    masked_source = source.copy() * mask_a

    lpa = LaplacianPyramid(masked_target, level)
    lpb = LaplacianPyramid(masked_source, level)

    blended = blend_pyramid(lpa, lpb, mp, blend_level)
    transformed_image = reconstruct(blended)
    out_img = np.clip(transformed_image, 0, 255)
    # transformed_image[transformed_image > 255] = 255
    # transformed_image[transformed_image < 0] = 0

    #out_img = source * (1 - mask) + (transformed_image * mask)
